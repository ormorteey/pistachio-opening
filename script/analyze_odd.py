...
def odd(Y, probs=False):
    """
    Also known as Shannon Odd
    Reference: https://en.wikipedia.org/wiki/Odd_(information_theory)
    Calculates the odd of a dataset Y. If probs is True, Y is treated as a set of probabilities.
    """
    # Y = np.array(Y)
    # if probs:
    #     if np.sum(Y) != 1:
    ...
    #     non_zero_prob = Y[Y > 0]
    #     odd_value = np.sum(-1 * non_zero_prob * np.log(non_zero_prob))
    #     return odd_value
    # else:
    ...
    #     # Compute odd for non-zero probabilities
    #     odd_value = np.sum(-1 * non_zero_prob * np.log(non_zero_prob))
...
def contigency_table(df, pred_cols, response_col, save=False, save_dir=None, file_type='xlsx', disable_tqdm=True):
    ...
    for col in tqdm(pred_cols, disable=disable_tqdm):
        ...
        for category in curr_ct.index: 
            ...
            curr_ct.loc[category, "Odd"] = odd(row_counts, probs=True)
            i +=1
...
def simulate_contigency_table(ct, ct_type, i=0, save=False, save_dir=None, file_type='xlsx'):
    ...
    for category in df.index:  
        ...
        df.loc[category, "Odd"] = odd(row_counts, probs=True)
...
def ensemble_ct(ct, n_samples, ct_type, disable_tqdm=True, save=True, save_dir=None, file_type='xlsx'):
    ...
    for i in tqdm(range(n_samples), disable=disable_tqdm):
        ...
        odd_list.append(sim_ct.loc[ rows_before_total, "Odd"])
    
    odd_df = pd.concat(odd_list, axis=1)
    odd_df.columns = list(range(n_samples))
    if save:
        if save_dir is None:
            file_name = f'odd_simulation_{ct_type}_alpha_{alpha}.{file_type}'
        else:
            os.makedirs(save_dir, exist_ok=True)
            file_name = os.path.join(save_dir, f'odd_simulation_{ct_type}.{file_type}')
        if file_type == 'xlsx':
            odd_df.to_excel(file_name)
        elif file_type == 'csv':
            odd_df.to_csv(file_name)
        else:
            raise ValueError("file_type must be either 'xlsx' or 'csv'")
...
def ensemble_ct_CI(df, ct, ct_type, alpha=0.05, save=False, save_dir=None, file_type='xlsx'):
    ...
    quantiles = ct_.loc[rows_before_total, "Odd"].to_frame().join(quantiles)
    quantiles['alpha'] = alpha
    # quantiles['alpha_sigf'] = (quantiles.apply(lambda row: row['Odd'] < row[f'{lwr_qtl*100}% Quantile'] or
    #                                             row['Odd'] > row[f'{upr_qtl*100}% Quantile'], axis=1))
    ...
    for idx in df_.index:
        ...
        observed_value = ct_.loc[idx, 'Odd']
        
        probabilities.loc[idx, 'Pr( <= Odd)'] = np.mean(simulated_values <= observed_value)
        probabilities.loc[idx, 'Pr( >= Odd)'] = np.mean(simulated_values >= observed_value)
        pval = np.min([probabilities.loc[idx, 'Pr( <= Odd)'], probabilities.loc[idx, 'Pr( >= Odd)']])
        probabilities.loc[idx, 'pval'] = pval
        probabilities.loc[idx, 'alpha_sigf'] = pval < alpha
        probabilities.loc[idx, 'ent(Y|x) < ent(Y)'] = observed_value < ct_.at[(idx[0], 'Total'), 'Odd']
...
def simulation_hist(sim_null, ct_null,
    ...
    entropies_null = ct_null['Odd'] 
    entropies_proper = ct_proper['Odd']

    y_odd = ct['Odd'].tolist()[-1]
    for idx, ((index_null, row_null), (index_proper, row_proper)) in enumerate(zip(sim_null.iterrows(), sim_proper.iterrows())):
        ...
        odd_null = entropies_null.iloc[idx]
        ax_null.axvline(x=odd_null, color='lime', linestyle='dashed', linewidth=2, label=f'Odd = {odd_null:.2f}')
        ax_null.axvline(x=y_odd, color='orange', linestyle='dashed', linewidth=2, label=f'Y-Odd = {y_odd:.2f}')
        ...
        odd_proper = entropies_proper.iloc[idx]
        ax_proper.axvline(x=odd_proper, color='lime', linestyle='dashed', linewidth=3, label=f'Odd = {odd_proper:.2f}')
        ax_proper.axvline(x=y_odd, color='orange', linestyle='dashed', linewidth=3, label=f'Y-Odd = {y_odd:.2f}')
        ax_proper.legend()
    ...
    for fig_num in range(num_figures):
        ...
        for ax_index in range(nrows * ncols):
            if plot_count < num_plots:
                ...
                odd_null = ct_null['Odd'].iloc[plot_count]
                
                ax.axvline(x=odd_null, color='lime', linestyle='--', label=f"Odd {odd_null:.2f}")
                ax.axvline(x=y_odd, color='orange', linestyle='--', label=f"Y-Odd {y_odd:.2f}")
                ...
                # Generate legend
                ax.legend(custom_lines, ['Null', 'Proper', f"Odd {odd_null:.2f}", f"Y-Odd {y_odd:.2f}"])
...
def make_cat_dataset(df, cols, bins, save=False, save_path=None):
    ...
    file_path = file_path + f'_cat_quantile_default{ext}'
...
def make_combined_df(df, num_combine, cols=None, sep=', ', categorical=True, bin=None, quantiles_to_check='default', save=False, save_path=None, verbose=True):
    ...
    if save:
        ...
        num_combine_filepath = file_path + f'_transformed_{num_combine}_features_{quantiles_to_check}_quantiles{ext}'
        num_combine_cat_filepath = file_path + f'_transformed_cat_{num_combine}_features_{quantiles_to_check}_quantiles{ext}'

        df_combined.to_csv(num_combine_filepath)
        df_combined_cat.to_csv(num_combine_cat_filepath)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Run significance analysis')
...
# In[ ]:
    ...
    if combine and (num_combine > 1):
        ...
        if os.path.exists(num_combine_filepath) and os.path.exists(num_combine_cat_filepath):
            print("========================================================================")
            print(f"\nLoading {num_combine_filepath}")
            transformed_data = pd.read_csv(num_combine_filepath, index_col=0)
            transformed_data_recoded = pd.read_csv(num_combine_cat_filepath, index_col=0)
            pred_cols = sorted(list(set(transformed_data_recoded.columns).difference(set(not_pred_cols))))
            print(f"\nLoaded {num_combine_filepath}")
        else:
            ...
            transformed_data, transformed_data_recoded, pred_cols = make_combined_df(data_df_to_combine, num_combine=num_combine, cols=pred_cols, sep=', ', quantiles_to_check=quantiles_to_check, categorical=categorical, bin=3, save=save, save_path=filepath, verbose=True)
