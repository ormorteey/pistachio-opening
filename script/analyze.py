...
def entropy(Y, probs=False):
    ...
    if probs:
        ...
        non_zero_prob = Y[Y > 0]
        entropy_value = np.sum(-1 * non_zero_prob * np.log2(non_zero_prob))
...
#-------------------------------------------------------------------
def plot_distributions_with_error_areas(sim_null, sim_proper, n_samples, idx, bins=20, save_dir=None, file_type='png', save=False, show=False):
    ...
    plt.xlabel('Value')
    plt.ylabel('Density')
    plt.title(f'Category {idx[1]} of {idx[0]} With Overlap Area = {overlap_area:.4f}')
    plt.legend()
    # Save the figure
    if save:
        if save_dir is None:
            file_name = f'Error_areas_Category {idx[1]} of {idx[0]}.png'
        else:
            os.makedirs(save_dir, exist_ok=True)
            file_name = os.path.join(save_dir, f'Error_areas_Category {idx[1]} of {idx[0]} with {overlap_area} overlapping area.png')
...
def add_min_sum_errors(sim_null, sim_proper, quantiles, n_samples, alpha, save=False, save_dir=None, file_type='xlsx'):
    for idx in sim_null.index:
        ...
        min_sum_errors = plot_distributions_with_error_areas(sim_null.loc[idx, :], sim_proper.loc[idx, :], n_samples, idx, save_dir=save_dir, file_type='png', save=True, show=False)
...
def simulation_hist(sim_null, ct_null,
                    ...
                    file_type='png', save=False, save_dir=None, per_fig_height=4,
...
def binary_bipartite_network_clusterplot(df, response_col, method='ward', metric='euclidean',
    ...
    row_linkage = linkage(pdist(df_, metric), method)
    col_linkage = linkage(pdist(df_.T, metric), method)

    # Cluster assignments
    if n_clusters_row:
        df['row_cluster'] = fcluster(row_linkage, n_clusters_row, criterion='maxclust')
    if n_clusters_col:
        df_['col_cluster'] = fcluster(col_linkage, n_clusters_col, criterion='maxclust').tolist()
    ...
    g = sns.clustermap(df_, method=method, metric=metric, cmap='flare', figsize=figsize, 
    ...
    if return_clusters:
        if flip:
            return df.T, g  # Return transposed DataFrame if flipped
        return df, g
...
def plot_combined(sim_null, sim_proper, ct_null, ct_proper, ct, nrows, ncols, file_type='png', save=False, save_dir=None, per_fig_height=4, show=True):
...
def make_plot_dendogram(df, response_color_cols,  cluster_col, cluster_color_col,
    ...
    if save:
        if save_dir is None:
            file_name = f'dendogram.png'
        else:
            os.makedirs(save_dir, exist_ok=True)
            file_name = os.path.join(save_dir, f'dendogram.png')
...
def process_pred_col(pred_col, df, response_col, n_samples, alpha, sigf_col,
    ...
    try:
        simulation_hist(sim_df_null, ct_ci_df_null,
                    ...
                    file_type='png', save=save, save_dir=save_dir_, per_fig_height=4,
...
def run_experiment(df, pred_cols, response_col, n_samples, alpha, sigf_col, do_summary=True,
                save=False, save_dir=None, generate_files_only=False, leaf_label_col=None,
                   use_checkpoint=True, file_type='xlsx', only_sigf=False, show_all_labels=False, max_workers=8):
    df_ = df.copy(deep=True)
    ...
    bb_net_clustermap = binary_bipartite_network_clusterplot(bb_net_null_df, response_col=response_col, method='ward', metric='euclidean', 
                                                          show_all_labels=show_all_labels, return_clusters=True, t=None, n_clusters=3,
                                                        figsize=(30, 30), save=True, save_dir=save_dir, file_type='png', title='', flip=True)
...
# In[ ]:
    ...
    run_experiment(df=transformed_data_df, pred_cols=pred_cols, response_col=response_col, n_samples=n_samples,
                    ...
                    generate_files_only=generate_files_only, use_checkpoint=use_checkpoint, file_type=file_type,
